...
export default class Menu {
  ...
  onStart(options: MenuOptions) {
    ...
    this._textColorButtonColorRectangleElement = utils.getElement(
      'menu-button-text-color-rectangle-color',
    );
    this._backgroundColorButtonColorRectangleElement = utils.getElement(
      'menu-button-background-color-rectangle-color',
    );

    const textSizeWindow = new TextSizeWindow(
      'menu-editor-text-size',
      this._handleOnTextSizeChangeRequested,
    );
    ...
    this._menuWindows = {
      download: new DownloadWindow(
        ...
        this._handleOnGenerateImage,
        this._handleonGenerateFileName,
      ),
      textSize: textSizeWindow,
      imageSize: imageSizeWindow,
    ...
    const setMenuToggleHandler = (buttonElement, menuWindow: MenuWindow) =>
      buttonElement.addEventListener('click', () =>
        this.onToggleMenuWindow(buttonElement, menuWindow),
      );
    ...
    setMenuToggleHandler(
      this.buttonElements.textSize,
      this._menuWindows.textSize,
    );
    setMenuToggleHandler(
      this.buttonElements.imageSize,
      this._menuWindows.imageSize,
    );
    setMenuToggleHandler(
      this.buttonElements.download,
      this._menuWindows.download,
    );

    this.buttonElements.info.addEventListener(
      'click',
      this._callbacks.onInfoButtonClicked,
    );
  }
  ...
  closeAllWindows = () => {
    utils
      .kvoIndexed(this.buttonElements)
      .forEach(({ value }: { value: HTMLElement }) =>
        value.classList.remove('active-button'),
      );

    utils.objForEach(this._menuWindows, (menuWindow: MenuWindow) =>
      menuWindow.close(),
    );
  ...
  _handleOnImageSizeChangeRequested = (
    width: number,
    height: number,
    scale: number,
  ) => {
  ...
  onToggleMenuWindow = (
    ...
    shouldMenuOpen: ?boolean = null,
  ) => {
    if (shouldMenuOpen == null) {
      const isMenuOpen: boolean = this._element.classList.contains(
        'menu-active',
      );
